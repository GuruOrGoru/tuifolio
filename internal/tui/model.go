...
func NewModel() *Model {
	vp := viewport.New(0, 0)
	vp.SetContent("Welcome to the Home tab!")
	return &Model{
		Cursor:        0,
...
func (m *Model) View() string {
	...
	innerHeight := len(m.Choices) + 5
	if outerHeight < innerHeight+2 {
	...
	case TabSettings:
		sb.WriteString("Settings: adjust your preferences.\n")
	case TabHelp:
		sb.WriteString("Help: press q to quit, ←/→ or 1/2/3 to switch tabs.\n")
	}
	...
	return CenterSquareWithContent(m.Terminal.Width, m.Terminal.Height, outerWidth, outerHeight, innerWidth, innerHeight, sb.String())
}
